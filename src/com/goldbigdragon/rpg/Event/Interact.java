...
public class Interact
{
	...
	public void PlayerInteractEntity (PlayerInteractEntityEvent event)
	{
		...
		if(target.getType() == EntityType.PLAYER)
		{
			...
			if(t.isOnline()==true)
			{
				com.goldbigdragon.rpg.gui.EquipGUI EGUI = new com.goldbigdragon.rpg.gui.EquipGUI();
	    ...
	    if(Main.UserData.containsKey(player))
		{
			...
			{
		    	com.goldbigdragon.rpg.etc.Quest Q = new com.goldbigdragon.rpg.etc.Quest();
		...
		if(player.getItemInHand() != null && player.getItemInHand().hasItemMeta() == true)
		{
			if(event.getItem().getTypeId() == 383 && event.getItem().getData().getData() == 0)
			{
				...
				for(int count = 0; count < monsterlist.length;count++)
		    	{
		    		...
		    		{
		    			new com.goldbigdragon.rpg.etc.Monster().SpawnMob(event.getClickedBlock().getLocation(), ChatColor.stripColor(event.getItem().getItemMeta().getDisplayName()));
						return;
		    		}
		    	}
	...
	private void Dungeon(PlayerInteractEvent event)
	{
		...
		com.goldbigdragon.rpg.effect.Particle p = new com.goldbigdragon.rpg.effect.Particle();
	...
	private void OPwork_Quest(PlayerInteractEvent event)
	{
		...
		if(event.getAction()==Action.LEFT_CLICK_BLOCK)
		{
			if(Main.UserData.get(player).getString((byte)1)=="TelePort")
			{
		    	...
		    	new com.goldbigdragon.rpg.effect.Sound().SP(event.getPlayer(), org.bukkit.Sound.ITEM_PICKUP, 0.5F,1.2F);
	...
	private void OPwork_Area(PlayerInteractEvent event)
	{
		...
		if(event.getAction()==Action.LEFT_CLICK_BLOCK)
		{
			if(Main.UserData.get(player).getString((byte)3)=="ANBI")
			{
				...
				new com.goldbigdragon.rpg.effect.Sound().SP(player, Sound.HORSE_SADDLE, 1.0F, 1.8F);
		...
		else if(event.getAction()==Action.RIGHT_CLICK_BLOCK)
		{
			if(Main.UserData.get(player).getString((byte)3)=="MLS")//MonsterLocationSetting
			{
				...
				AreaConfig.saveConfig();
				new com.goldbigdragon.rpg.effect.Sound().SP(player, Sound.ITEM_PICKUP, 1.0F, 1.8F);
		    	Main.UserData.get(player).clearAll();
	...
	public void PlayerGetItem(PlayerPickupItemEvent event)
	{
		YamlController Event_YC = Main.Event_YC;
	  	if(Event_YC.isExit("Stats/" + event.getPlayer().getUniqueId()+".yml") == false)
	  		new com.goldbigdragon.rpg.config.StatConfig().CreateNewStats(event.getPlayer());
		...
		if((event.getItem().getItemStack().hasItemMeta() == true && event.getItem().getItemStack().getItemMeta().hasDisplayName()))
		{
			...
			if(ItemName.contains(ServerOption.Money) == true && event.getItem().getItemStack().getEnchantmentLevel(Enchantment.LUCK) == 500)
			{
			  	if(Event_YC.isExit("Stats/" + event.getPlayer().getUniqueId()+".yml") == false)
			  		new com.goldbigdragon.rpg.config.StatConfig().CreateNewStats(event.getPlayer());
		...
		{
			if(ItemName.contains(ServerOption.Money) == true)
				new com.goldbigdragon.rpg.effect.PacketSender().sendActionBar(event.getPlayer(), ChatColor.GRAY+""+ChatColor.BOLD+""+""+ChatColor.BOLD+ChatColor.BOLD+""+ChatColor.BOLD+"("+""+ChatColor.BOLD+ItemName+""+ChatColor.GRAY+""+ChatColor.BOLD+")");
			else
				new com.goldbigdragon.rpg.effect.PacketSender().sendActionBar(event.getPlayer(), ChatColor.GRAY+""+ChatColor.BOLD+""+""+ChatColor.BOLD+ChatColor.BOLD+""+ChatColor.BOLD+"("+""+ChatColor.BOLD+ItemName+""+" "+ChatColor.GRAY+""+ChatColor.BOLD+event.getItem().getItemStack().getAmount()+"개)");
		}
	  	return;
	}
