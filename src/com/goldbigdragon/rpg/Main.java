...
public class Main extends JavaPlugin implements Listener
{
	...
	public void onEnable()
	{
		...
		if(!MusicFolder.exists())
	  	...
	  	com.goldbigdragon.rpg.config.configConfig config = new com.goldbigdragon.rpg.config.configConfig();
	  	config.CreateNewConfig(Main_YC);
	  	config.CreateMapImageConfig(Main_YC);
	  	if(Main_YC.isExit("Skill/SkillList.yml") == false)
	  	  new com.goldbigdragon.rpg.config.SkillConfig().CreateNewSkillList();
	  	if(Main_YC.isExit("Skill/JobList.yml") == false)
	  		new com.goldbigdragon.rpg.config.SkillConfig().CreateNewJobList();
	  	if(Main_YC.isExit("ETC/NewBie.yml") == false)
	  		new com.goldbigdragon.rpg.config.NewBieConfig().CreateNewConfig();
	  	new com.goldbigdragon.rpg.config.NPCconfig().NPCscriptExample();
		...
		new com.goldbigdragon.rpg.servertick.ServerTickMain(this);
		new com.goldbigdragon.rpg.servertick.ServerTickScheduleManager().loadCategoriFile();
	...
	public void onDisable()
	{
		if(Bukkit.getPluginManager().isPluginEnabled("NoteBlockAPI"))
		{
	    	...
	    	for(int count = 0; count <a.length;count++)
	    		new com.goldbigdragon.rpg.connector.NoteBlockAPIMain().Stop(a[count]);
		}
	  	new UserDataManager().saveCategoriFile();
	  	new PartyDataManager().saveParty();

		new com.goldbigdragon.rpg.servertick.ServerTickScheduleManager().saveCategoriFile();
	...
	private void PlayerJoin(PlayerJoinEvent event)
	{
		...
		{
			com.goldbigdragon.rpg.effect.PacketSender PS = new com.goldbigdragon.rpg.effect.PacketSender();
			String alert = "[";
		...
		if(player.isOp() == true)
		{
	  	...
	  	{
	  	    com.goldbigdragon.rpg.config.StatConfig stat = new com.goldbigdragon.rpg.config.StatConfig();
	  		stat.CreateNewStats(player);
	  	}
	  	if(Main_YC.isExit("Quest/PlayerData/" + player.getUniqueId()+".yml") == false)
	  	{
	  	    com.goldbigdragon.rpg.config.QuestConfig quest = new com.goldbigdragon.rpg.config.QuestConfig();
					...
					if(QuestName.compareTo(Quest[count].toString())==0)
					{
						if(QuestList.getConfigurationSection(QuestName+".FlowChart").getKeys(false).toArray().length != 0)
						{
							...
							if(QuestList.getString(QuestName+".FlowChart."+0+".Type").compareTo("Nevigation")==0||
								...
								new com.goldbigdragon.rpg.gui.QuestGUI().QuestTypeRouter(player, QuestName);
	  	...
	  	{
	  	    com.goldbigdragon.rpg.config.SkillConfig skill = new com.goldbigdragon.rpg.config.SkillConfig();
	  	  skill.CreateNewPlayerSkill(player);
	  	}
		com.goldbigdragon.rpg.etc.Job J = new com.goldbigdragon.rpg.etc.Job();
		J.FixJobList();
		J.FixPlayerJobList(player);
		J.FixPlayerSkillList(player);
		
    	com.goldbigdragon.rpg.util.ETC ETC = new com.goldbigdragon.rpg.util.ETC();
    	ETC.UpdatePlayerHPMP(event.getPlayer());
    	new com.goldbigdragon.rpg.gui.EquipGUI().FriendJoinQuitMessage(player, true);
	...
	private void PlayerQuit(PlayerQuitEvent event)
	{
		...
		{
			com.goldbigdragon.rpg.connector.NoteBlockAPIMain NBAPIM = new com.goldbigdragon.rpg.connector.NoteBlockAPIMain();
    	...
    	new com.goldbigdragon.rpg.gui.EquipGUI().FriendJoinQuitMessage(player, false);
	...
	private void Move(PlayerMoveEvent event){
		com.goldbigdragon.rpg.event.PlayerAction PA = new com.goldbigdragon.rpg.event.PlayerAction();PA.PlayerMove(event);return;}
	...
	private void HotBarMove(PlayerItemHeldEvent event)
	{
		if(MagicSpellsCatched == false)
			MagicSpellCatch();
		 new com.goldbigdragon.rpg.event.ChangeHotBar().HotBarMove(event);
		 return;
	}
	...
	private void PlayerChatting(PlayerChatEvent event)
	{
		...
		com.goldbigdragon.rpg.event.PlayerAction PA = new com.goldbigdragon.rpg.event.PlayerAction();
	...
	{
		com.goldbigdragon.rpg.event.Fishing F = new com.goldbigdragon.rpg.event.Fishing();
	...
	{
		new com.goldbigdragon.rpg.etc.Map().onMap(event);
		return;
	}
	...
	public void MagicSpellCatch()
	{
		if(MagicSpellsCatched == false)
		{
			...
			{
				com.goldbigdragon.rpg.connector.SpellMain MS = new com.goldbigdragon.rpg.connector.SpellMain(this);
			}
		}
		return;
	}
	...
	public void CitizensCatch()
	{
		if(CitizensCatched == false)
		{
			...
			{
				com.goldbigdragon.rpg.connector.CitizensMain CZ = new com.goldbigdragon.rpg.connector.CitizensMain(this);
			}
		}
		return;
	}
	...
	{
		if(Bukkit.getPluginManager().isPluginEnabled("NoteBlockAPI"))
			new com.goldbigdragon.rpg.connector.NoteBlockAPIMain(this);
		return;
	}
	...
	@EventHandler
	private void RA(EntityShootBowEvent event) {new com.goldbigdragon.rpg.event.Attack().RangeAttack(event);return;}
    @EventHandler
    private void Attack(EntityDamageByEntityEvent event) {new com.goldbigdragon.rpg.event.Attack().AttackRouter(event);return;}
    @EventHandler
	private void EntitySpawn(CreatureSpawnEvent event) {new com.goldbigdragon.rpg.event.MonsterSpawn().EntitySpawn(event);return;}
    @EventHandler
    private void ITBlock(PlayerInteractEvent event)
    {
    	com.goldbigdragon.rpg.util.ETC ETC = new com.goldbigdragon.rpg.util.ETC();
    	ETC.UpdatePlayerHPMP(event.getPlayer());
    	new com.goldbigdragon.rpg.event.Interact().PlayerInteract(event);
    	return;
    }
    @EventHandler
    private void ITEnity(PlayerInteractEntityEvent event)
    {
    	...
    	ETC.UpdatePlayerHPMP(event.getPlayer());
    	CitizensCatch();
    	new com.goldbigdragon.rpg.event.Interact().PlayerInteractEntity(event);
    	return;
    }
    @EventHandler
    private void ItemGetMessage(PlayerPickupItemEvent event) {new com.goldbigdragon.rpg.event.Interact().PlayerGetItem(event);}
	@EventHandler
	private void MonsterKill(EntityDeathEvent event)	{new com.goldbigdragon.rpg.event.MonsterKill().MonsterKill(event);return;}
	@EventHandler
	private void BBreak(BlockBreakEvent event)
	{
	    com.goldbigdragon.rpg.event.BlockBreak BB = new com.goldbigdragon.rpg.event.BlockBreak();
		BB.BlockBreaking(event);return;
	}
	@EventHandler
	private void BlockPlace(BlockPlaceEvent event){
		com.goldbigdragon.rpg.event.BlockPlace BP = new com.goldbigdragon.rpg.event.BlockPlace();BP.BlockPlace(event);return;}
	...
	private void InventoryClick(InventoryClickEvent event)
	{
		...
		if(event.getInventory().getName().length() >= 3)
		{
			...
			&&event.getInventory().getName().charAt(1)=='0')
			{
				...
				{
					com.goldbigdragon.rpg.event.InventoryClick IC = new com.goldbigdragon.rpg.event.InventoryClick();
	...
	private void InventoryClose(InventoryCloseEvent event)
	{
		...
		new com.goldbigdragon.rpg.event.InventoryClose().InventoryCloseRouter(event);
		return;
	}
		...
		if(talker instanceof Player)
		{
			...
			case"테스트":
				...
				case"친구":
				 	...
				 	new com.goldbigdragon.rpg.gui.ETCGUI().FriendsGUI(player, 0);
					return true;
				case"스킬":
				 	s.SP((Player)talker, org.bukkit.Sound.HORSE_SADDLE, 1.0F, 1.8F);
				...
				case "아이템제거":
					...
					return true;
		  		case "아이템" :
		  			com.goldbigdragon.rpg.command.ItemsCommand ItemC = new com.goldbigdragon.rpg.command.ItemsCommand();
		  			if(args.length <= 0)
		  			{
		  				if(args.length <=0)
		  				{
		  					com.goldbigdragon.rpg.command.HelpMessage HM = new com.goldbigdragon.rpg.command.HelpMessage();
		  			...
		  			break;
		  		case "파티":
		  			com.goldbigdragon.rpg.command.PartyCommand PartyC = new com.goldbigdragon.rpg.command.PartyCommand();
		  		...
		  		case "몬스터" :
					  s.SP((Player)talker, org.bukkit.Sound.HORSE_ARMOR, 0.8F, 1.8F);
		  			new com.goldbigdragon.rpg.gui.MonsterGUI().MonsterListGUI(player, 0);
		  			return true;
		  		case "워프":
		  			com.goldbigdragon.rpg.command.WarpCommand WarpC = new com.goldbigdragon.rpg.command.WarpCommand();
		  			WarpC.onCommand(talker, command, string, args);
		  			return true;
		  		case "영역":
		  			com.goldbigdragon.rpg.command.AreaCommand AreaC = new com.goldbigdragon.rpg.command.AreaCommand();
		  			AreaC.onCommand(talker, command, string, args);
		  			return true;
		  		case "상점":
		  			com.goldbigdragon.rpg.command.NPCcommand NPCC = new com.goldbigdragon.rpg.command.NPCcommand();
		  			NPCC.onCommand(talker, command, string, args);
		  			return true;
		  		case "퀘스트":
		  			com.goldbigdragon.rpg.command.QuestCommand QC = new com.goldbigdragon.rpg.command.QuestCommand();
