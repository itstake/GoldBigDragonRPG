...
public class PlayerAction
{
	public void PlayerMove(PlayerMoveEvent event)
	{
		...
		//player.teleport(event.getPlayer().getLocation());
	  	...
	  	if(Config_YC.isExit("Stats/" + player.getUniqueId()+".yml") == false)
	  		new com.goldbigdragon.rpg.config.StatConfig().CreateNewStats(player);
		...
		if(Area != null)
		{
			new com.goldbigdragon.rpg.etc.Area().AreaMonsterSpawnAdd(Area, "-1");
			if(Main.PlayerCurrentArea.get(player).equals(Area)==false)
			{
				if(Bukkit.getPluginManager().isPluginEnabled("NoteBlockAPI") == true)
					new com.goldbigdragon.rpg.connector.NoteBlockAPIMain().Stop(player);
				...
				if(A.getAreaOption(Area, (char) 6) == true)
				{
					...
					if(Bukkit.getPluginManager().isPluginEnabled("NoteBlockAPI") == true)
						new com.goldbigdragon.rpg.connector.NoteBlockAPIMain().Play(player, AreaList.getInt(Area+".BGM"));
				}
		...
		else
		{
			...
			{
				for(int count = 0; count < com.goldbigdragon.rpg.servertick.ServerTickMain.MobSpawningAreaList.size(); count++)
				{
					if(PrevArea.equals(com.goldbigdragon.rpg.servertick.ServerTickMain.MobSpawningAreaList.get(count)))
						com.goldbigdragon.rpg.servertick.ServerTickMain.MobSpawningAreaList.remove(count);
				}
			}
			return;
		}
		...
		if(Main.TEMP.get(player).equals("FA"))
		{
			...
			else
			{
				...
				if(Bukkit.getServer().getPlayer(Message) != null)
					new com.goldbigdragon.rpg.gui.EquipGUI().SetFriends(player, Bukkit.getServer().getPlayer(Message));
				else
				{
					s.SP(player, Sound.ORB_PICKUP, 1.0F, 1.8F);
	...
	private void QuestTypeChatting(PlayerChatEvent event)
	{
    	...
    	case "BPID"://BlockPlaceID
			...
			{
				com.goldbigdragon.rpg.event.Interact I = new com.goldbigdragon.rpg.event.Interact();
	...
	private void ItemTypeChatting(PlayerChatEvent event)
	{
		...
		com.goldbigdragon.rpg.gui.UpGradeGUI UpGUI = new com.goldbigdragon.rpg.gui.UpGradeGUI();
		com.goldbigdragon.rpg.effect.Sound s = new com.goldbigdragon.rpg.effect.Sound();
		...
		switch(Main.UserData.get(player).getString((byte)1))
		{
			...
			case "HP":
				if(isIntMinMax(Message, player, Integer.MIN_VALUE, Integer.MAX_VALUE))
				{
					...
					if(Main.UserData.get(player).getInt((byte)4) != -1)
					{
						...
						{
							com.goldbigdragon.rpg.gui.UseableItemGUI UGUI = new com.goldbigdragon.rpg.gui.UseableItemGUI();
				...
				if(isIntMinMax(Message, player, Integer.MIN_VALUE, Integer.MAX_VALUE))
				{
					...
					if(Main.UserData.get(player).getInt((byte)4) != -1)
					{
						...
						else
						{
							...
							player.sendMessage(ChatColor.DARK_AQUA+"[아이템] : 아이템의 보너스 "+ ServerOption.STR+"을 입력해 주세요!");
							player.sendMessage(ChatColor.DARK_AQUA+"(-127 ~ 127)");
							return;
						}
					}
			...
			case "STR":
				if(isIntMinMax(Message, player, -127, 127))
				{
					...
					player.sendMessage(ChatColor.DARK_AQUA+"[아이템] : 아이템의 보너스 "+ ServerOption.DEX+"를 입력해 주세요!");
					player.sendMessage(ChatColor.DARK_AQUA+"(-127 ~ 127)");
				}
				return;
			case "DEX":
				if(isIntMinMax(Message, player, -127, 127))
				{
					...
					player.sendMessage(ChatColor.DARK_AQUA+"[아이템] : 아이템의 보너스 "+ ServerOption.INT+"을 입력해 주세요!");
					player.sendMessage(ChatColor.DARK_AQUA+"(-127 ~ 127)");
				}
				return;
			case "INT":
				if(isIntMinMax(Message, player, -127, 127))
				{
					...
					player.sendMessage(ChatColor.DARK_AQUA+"[아이템] : 아이템의 보너스 "+ ServerOption.WILL+"를 입력해 주세요!");
					player.sendMessage(ChatColor.DARK_AQUA+"(-127 ~ 127)");
				}
				return;
			case "WILL":
				if(isIntMinMax(Message, player, -127, 127))
				{
					...
					player.sendMessage(ChatColor.DARK_AQUA+"[아이템] : 아이템의 보너스 "+ ServerOption.LUK+"을 입력해 주세요!");
					player.sendMessage(ChatColor.DARK_AQUA+"(-127 ~ 127)");
				}
				return;
			case "LUK":
		...
		||Main.UserData.get(player).getType().equals("UseableItem"))
			new com.goldbigdragon.rpg.gui.UseableItemGUI().NewUseableItemGUI(player, number);
		else
			IGUI.NewItemGUI(player, number);
		Main.UserData.get(player).clearAll();
		return;
			...
			case "AMSM"://AreaMonsterSpawnMonster
				...
				if(answer!=-1)
				{
					if(answer==0)
					{
		    			...
		    			new com.goldbigdragon.rpg.etc.Area().AreaMonsterSpawnAdd(AreaName, Main.UserData.get(player).getString((byte)1));
					}
					else
					{
	...
	private void NPCTypeChatting(PlayerChatEvent event)
	{
		...
		YamlController GUI_YC = Main.GUI_YC;
	  	...
	  	{
	  		com.goldbigdragon.rpg.config.NPCconfig NPCC = new com.goldbigdragon.rpg.config.NPCconfig();
		...
		case "NUC"://NPC'sUpgradeCost
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				com.goldbigdragon.rpg.gui.NPC_GUI NGUI = new com.goldbigdragon.rpg.gui.NPC_GUI();
	...
	private void JobTypeChatting(PlayerChatEvent event)
	{
		...
		case "CC"://CreateCategory
			if(JobList.getConfigurationSection("Mabinogi").getKeys(false).toString().contains(Message) == false)
			{
				...
				new com.goldbigdragon.rpg.etc.Job().AllPlayerFixAllSkillAndJobYML();
				Main.UserData.get(player).clearAll();
			}
			else
			{
		...
		case "JNL"://JobNeedLevel (승급을 위한 필요 레벨)
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				sound.SP(player, org.bukkit.Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.LIGHT_PURPLE + "[직업] : "+ChatColor.YELLOW +JobNick2+ChatColor.LIGHT_PURPLE +"의 승급 필요 "+ ServerOption.STR+"을 설정하세요.");
			}
			return;
		case "JNS" : 
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				sound.SP(player, org.bukkit.Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.LIGHT_PURPLE + "[직업] : "+ChatColor.YELLOW +JobNick2+ChatColor.LIGHT_PURPLE +"의 승급 필요 "+ ServerOption.DEX+"를 설정하세요.");
			}
			return;
		case "JND" : 
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				sound.SP(player, org.bukkit.Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.LIGHT_PURPLE + "[직업] : "+ChatColor.YELLOW +JobNick2+ChatColor.LIGHT_PURPLE +"의 승급 필요 "+ ServerOption.INT+"을 설정하세요.");
			}
			return;
		case "JNI" : 
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				sound.SP(player, org.bukkit.Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.LIGHT_PURPLE + "[직업] : "+ChatColor.YELLOW +JobNick2+ChatColor.LIGHT_PURPLE +"의 승급 필요 "+ ServerOption.WILL+"를 설정하세요.");
			}
			return;
		case "JNW" : 
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				sound.SP(player, org.bukkit.Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.LIGHT_PURPLE + "[직업] : "+ChatColor.YELLOW +JobNick2+ChatColor.LIGHT_PURPLE +"의 승급 필요 "+ ServerOption.LUK+"을 설정하세요.");
			}
			return;
		case "JNLU" : 
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
	...
	private void MonsterTypeChatting(PlayerChatEvent event)
	{
		...
		case "STR"://Strength
			if(isIntMinMax(message, player, 0, Integer.MAX_VALUE))
			{
				...
				Monster.saveConfig();
				s.SP(player, Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.GRAY+"("+ ServerOption.DEX+"는 몬스터의 원거리 공격력을 상승시켜 줍니다.)");
				player.sendMessage(ChatColor.GREEN+"[몬스터] : 몬스터의 "+ ServerOption.DEX+"를 설정해 주세요!");
		...
		case "DEX"://DEX
			if(isIntMinMax(message, player, 0, Integer.MAX_VALUE))
			{
				...
				Monster.saveConfig();
				s.SP(player, Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.GRAY+"("+ ServerOption.INT+"은 몬스터의 폭발 공격력을 상승시켜 줍니다.)");
				player.sendMessage(ChatColor.GREEN+"[몬스터] : 몬스터의 "+ ServerOption.INT+"을 설정해 주세요!");
				player.sendMessage(ChatColor.DARK_AQUA+"(1 ~ "+Integer.MAX_VALUE+")");
				Main.UserData.get(player).setString((byte)1, "INT");
			}
			return;
		case "INT"://INT
			if(isIntMinMax(message, player, 0, Integer.MAX_VALUE))
			{
				...
				Monster.saveConfig();
				s.SP(player, Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.GRAY+"("+ ServerOption.WILL+"는 몬스터의 크리티컬 확률을 상승시켜 줍니다.)");
				player.sendMessage(ChatColor.GREEN+"[몬스터] : 몬스터의 "+ ServerOption.WILL+"를 설정해 주세요!");
				player.sendMessage(ChatColor.DARK_AQUA+"(1 ~ "+Integer.MAX_VALUE+")");
				Main.UserData.get(player).setString((byte)1, "WILL");
			}
			return;
		case "WILL"://WILL
			if(isIntMinMax(message, player, 0, Integer.MAX_VALUE))
			{
				...
				Monster.saveConfig();
				s.SP(player, Sound.ITEM_PICKUP, 1.0F, 1.0F);
				player.sendMessage(ChatColor.GRAY+"("+ ServerOption.LUK+"은 몬스터의 크리티컬 확률을 크게 상승시켜 줍니다.)");
				player.sendMessage(ChatColor.GREEN+"[몬스터] : 몬스터의 "+ ServerOption.LUK+"을 설정해 주세요!");
				player.sendMessage(ChatColor.DARK_AQUA+"(1 ~ "+Integer.MAX_VALUE+")");
				Main.UserData.get(player).setString((byte)1, "LUK");
			}
			return;
	...
	private void TeleportTypeChatting(PlayerChatEvent event)
	{
		...
		case "NW"://NewWarp
			message = message.replace(".", "");
			new com.goldbigdragon.rpg.etc.Teleport().CreateNewTeleportSpot(player, message);
			Main.UserData.get(player).clearAll();
			return;
		}
		return;
	...
	private void EventChatting(PlayerChatEvent event)
	{
		...
		case "SKP"://SkillPoint
			if(isIntMinMax(message, player, 1, 10))
			{
				...
				Config.saveConfig();
				new com.goldbigdragon.rpg.gui.EventGUI().EventGUI_Main(player);
				Main.UserData.get(player).clearAll();
			}
			return;
		case "STP"://StatPoint
	...
	private void SystemTypeChatting(PlayerChatEvent event)
	{
		...
		case "CCP"://ChangeChatPrefix
			...
			Config.saveConfig();
			Main.UserData.get(player).clearAll();
			new com.goldbigdragon.rpg.gui.OPBoxGUI().OPBoxGUI_Setting(player);
			return;
		case "BMT"://BroadcastMessageTick
			if(isIntMinMax(message, player, 1, 3600))
			{
				...
				Config.saveConfig();
				new com.goldbigdragon.rpg.gui.OPBoxGUI().OPBoxGUI_BroadCast(player, 0);
				Main.UserData.get(player).clearAll();
			}
			return;
		...
		case "CSN"://ChangeStatName
			{
				...
				new com.goldbigdragon.rpg.gui.OPBoxGUI().OPBoxGUI_StatChange(player);
			}
			return;
		}
		return;
	...
	private void NaviTypeChatting(PlayerChatEvent event)
	{
		...
		case "NN"://NewNavigation
			s.SP(player, Sound.ITEM_PICKUP, 1.0F, 1.0F);
			long UTC = new com.goldbigdragon.rpg.util.ETC().getNowUTC();
			...
			Main.UserData.get(player).clearAll();
			new com.goldbigdragon.rpg.gui.NavigationGUI().NavigationOptionGUI(player,UTC+"");
			return;
		case "CNN"://ChangeNavigationName이름 변경
			s.SP(player, Sound.ITEM_PICKUP, 1.0F, 1.0F);
			NavigationConfig.set(Main.UserData.get(player).getString((byte)1)+".Name", event.getMessage());
			NavigationConfig.saveConfig();
			new com.goldbigdragon.rpg.gui.NavigationGUI().NavigationOptionGUI(player,Main.UserData.get(player).getString((byte)1));
			Main.UserData.get(player).clearAll();
			return;
