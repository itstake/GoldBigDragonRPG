	...
	public void onCommand(CommandSender talker, Command command, String string, String[] args)
	{
		...
		if(talker.isOp() == true)
		{
			if(args.length==0)
			{
				...
				new com.goldbigdragon.rpg.gui.QuestGUI().MyQuestListGUI(player, 0);
				return;
			}
			...
			if(player.isOp() == true)
			{
				...
				YamlController Config_YC = com.goldbigdragon.rpg.Main.Config_YC;
				...
				switch(ChatColor.stripColor(args[0]))
				{
		  			...
		  			{
		  					s.SP(player, Sound.HORSE_ARMOR, 1.0F, 0.8F);
		  					new com.goldbigdragon.rpg.gui.QuestGUI().AllOfQuestListGUI(player, 0,false);
		  			}
		  			break;
			  		case "생성" :
				  		{
				  			...
				  			||args[1].equalsIgnoreCase("일주")||args[1].equalsIgnoreCase("한달"))
				  			{
							    ...
							    player.sendMessage(ChatColor.GREEN+"[SYSTEM] : "+ChatColor.YELLOW+QuestNameString+ChatColor.DARK_AQUA+" 퀘스트가 생성되었습니다!");
			  					s.SP(player, Sound.HORSE_ARMOR, 1.0F, 0.8F);
			  					new com.goldbigdragon.rpg.gui.QuestGUI().FixQuestGUI(player, 0, QuestNameString);
				  			}
				  			else
							HelpMessager((Player)talker,8);
				  		}
