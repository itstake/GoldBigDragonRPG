package com.goldbigdragon.rpg.servertick;
...
public class ServerTickMain
{
	...
	{
		nowUTC = new com.goldbigdragon.rpg.util.ETC().getNowUTC();
	...
	{
		YamlController Scheduler_YC = Main.Scheduler_YC;
		...
		if(BroadCastMessageCool>=BroadCastMessageTime)
		{
			...
			if(BroadCast.contains("0"))
				if(BroadCast.getConfigurationSection("").getKeys(false).toArray().length != 0)
					Bukkit.broadcastMessage(BroadCast.getString(BroadCast.getConfigurationSection("").getKeys(false).toArray()[new com.goldbigdragon.rpg.util.Number().RandomNum(0, BroadCast.getConfigurationSection("").getKeys(false).toArray().length-1)].toString()));
		}
		else
