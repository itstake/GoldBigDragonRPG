...
public class Area
{
	...
	public void sendAreaTitle(Player player, String AreaName)
	{
		...
		if(getAreaOption(AreaName, (char)4) == true)
		{
			...
			new com.goldbigdragon.rpg.effect.PacketSender().sendTitleSubTitle(player, "\'"+Title+"\'", "\'"+Description+"\'", 1, 10, 1);
		}
		return;
	}
		...
		if(Long.parseLong(Count)!=-1)
		{
			if(AreaConfig.contains(AreaName+".MonsterSpawnRule")==true)
			{
				if(AreaConfig.getConfigurationSection(AreaName+".MonsterSpawnRule").getKeys(false).size()!=0)
				{
					...
					{
						if(com.goldbigdragon.rpg.servertick.ServerTickMain.MobSpawningAreaList.contains(AreaName)==false)
							com.goldbigdragon.rpg.servertick.ServerTickMain.MobSpawningAreaList.add(AreaName);
						
						Long UTC = com.goldbigdragon.rpg.servertick.ServerTickMain.nowUTC+5;
						for(;;)
						{
							if(com.goldbigdragon.rpg.servertick.ServerTickMain.Schedule.containsKey(UTC))
								UTC=UTC+1;
							else
								break;
						}
						com.goldbigdragon.rpg.servertick.ServerTickScheduleObject OBJECT = new com.goldbigdragon.rpg.servertick.ServerTickScheduleObject(UTC, "A_MS");
						...
						com.goldbigdragon.rpg.servertick.ServerTickMain.Schedule.put(UTC, OBJECT);
					}
				}
			}
		}
