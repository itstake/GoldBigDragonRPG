			...
			if(Main.PartyJoiner.containsKey(player)==false)
			{
				...
				for(;;)
				{
					...
					else
					{
						...
						new com.goldbigdragon.rpg.effect.PacketSender().sendTitleSubTitle(player, "\'"+ChatColor.WHITE+"Level Up!\'", "\'"+ChatColor.WHITE + "레벨 " +ChatColor.YELLOW+ YM.getInt("Stat.Level")+ChatColor.WHITE + "이 되었습니다!\'", 1, 3, 1);
						new com.goldbigdragon.rpg.effect.Sound().SP(player, org.bukkit.Sound.LEVEL_UP, 1.5F, 1.8F);
						if(Config.getInt("Server.MaxLevel") <= YM.getInt("Stat.Level"))
				...
				if(player[count].getLocation().getWorld() == loc.getWorld())
				{
					if(loc.distance(player[count].getLocation()) <= Config.getInt("Party.EXPShareDistance"))
					{
					  	if(Event_YC.isExit("Stats/" + player[count].getUniqueId()+".yml") == false)
					  		new com.goldbigdragon.rpg.config.StatConfig().CreateNewStats(player[count]);
						...
						for(;;)
						{
							...
							else
							{
								...
								YM.set("Stat.StatPoint", YM.getInt("Stat.StatPoint") + (Config.getInt("Server.Level_Up_StatPoint")*Config.getInt("Event.Multiple_Level_Up_StatPoint")));
								YM.saveConfig();
								new com.goldbigdragon.rpg.effect.PacketSender().sendTitleSubTitle(player[count], "\'"+ChatColor.WHITE+"Level Up!\'", "\'"+ChatColor.WHITE + "레벨 " +ChatColor.YELLOW+ YM.getInt("Stat.Level")+ChatColor.WHITE + "이 되었습니다!\'", 1, 3, 1);
								new com.goldbigdragon.rpg.effect.Sound().SP(player[count], org.bukkit.Sound.LEVEL_UP, 1.5F, 1.8F);
							}
						}
						YM.saveConfig();
					}
